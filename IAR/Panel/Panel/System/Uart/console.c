...
//-----------------------------------------------------------------------------------
u8 console_mode=0;	// 0-Protocol mode 1-console mode
//-----------------------------------------------------------------------------------
u8	itmp,itmp1,itmp2;
...
__farflash u8	Help[Help_Len][Help_Word_Len]=
{
	"\n\r"
	"\n\r Monro S.A."
...
__farflash u8	Slovar[Slovar_Len][Slovar_Word_Len]=
{ 
	"set",				//0
	"show",				//1
	"store",			//2
...
void con_init()
{
	con_hardware_init(CONSOLE_BAUND);
	outptr=&outtxt[0];
	outtxt[0]=0;	
	word[0]=0;
	wordptr=&word[0];	
...
void con_parser()
{
	u16 i=0;	
	u8 *ptr;
	MenuPtr=(TMenuItem*)_Root.SubItem;
	in=intxt[0];
	Params_Count=0;
...
//						MenuList_Count++;					
						...
						{
							_sprintf(outtxt,"\n\r ");
							_sprintf(outtxt,StrLine);
							_sprintf(outtxt," : Available parameters ");
							MenuPtr2=MenuPtr->SubItem;
							while(MenuPtr2!=NULL)
							{
								_sprintf(outtxt,"\n\r   ");
								GetFlashStr(outtxt,(u8*)MenuPtr2->Word);
								MenuPtr2=MenuPtr2->SubItem;
							}
							_sprintf(outtxt,"\n\r>");
							MenuPtr=NULL;												
						}
