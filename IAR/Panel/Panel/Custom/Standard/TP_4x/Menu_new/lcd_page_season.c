...
void scr12_plus()
{
	...
	{
	case 1:	if(Season.t<400)Season.t++;break;
	case 2:	if(Season.date[0].field.Date<31)Season.date[0].field.Date++;break;
	case 3:	if(Season.date[0].field.Month<12)Season.date[0].field.Month++;break;
	case 4:	if(Season.date[1].field.Date<31)Season.date[1].field.Date++;break;
	case 5:	if(Season.date[1].field.Month<12)Season.date[1].field.Month++;break;
	}
	new_step=2;	
...
void scr12_minus()
{
	...
	{
	case 1:	if(Season.t>-400)Season.t--;break;
	case 2:	if(Season.date[0].field.Date>1)Season.date[0].field.Date--;break;
	case 3:	if(Season.date[0].field.Month>1)Season.date[0].field.Month--;break;
	case 4:	if(Season.date[1].field.Date>1)Season.date[1].field.Date--;break;
	case 5:	if(Season.date[1].field.Month>1)Season.date[1].field.Month--;break;
	}
	new_step=2;	
	Button_no_update_time=500;
	scr12_blink=1;
...
void scr12_update_season_buttons()
{
	...
	// Temperature switch
	if(Season.switch_temperature==1)
	{
...
{
	Season.switch_date=0;
	if(Season.switch_temperature)Season.switch_temperature=0;
	else Season.switch_temperature=1;
	scr12_update_season_buttons();
	new_step=2;
	Button_no_update_time=500;
}
...
{
	Season.switch_temperature=0;
	if(Season.switch_date)Season.switch_date=0;
	else Season.switch_date=1;
	scr12_update_season_buttons();	
	new_step=2;	
	Button_no_update_time=500;
}
...
//	u8 old_stat;
	...
	//-----------------------------------------------	
	// Real temperature
	temperature=Season.t;	// Real output temperature
	...
	temp=Season.date[0].field.Month;		
	...
	temp=Season.date[1].field.Month;		
