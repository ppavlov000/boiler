...
void work_to_regs()
{
	...
	{
		kotel.t=WorkInfo.temperature[0];
		return;
	}

	if(WorkInfo.type!=TYPE_KOTEL)return;	
	MenuRegs.Season.season=WorkInfo.season;
	kotel.mode=WorkInfo.mode;
	kotel.t=WorkInfo.temperature[0];
}
//-----------------------------------------------------------------------------------
void regs_to_work()
...
//	TWorkInfo *work_info=(TWorkInfo*)&tx.field.d[0];
	WorkInfo.type=TYPE_KOTEL;
	WorkInfo.season=MenuRegs.Season.season;
	WorkInfo.mode=kotel.mode;
	WorkInfo.temperature[0]=kotel.t;
	WorkInfo.temperature[1]=io.ainput[kotel.channel];	
	WorkInfo.temperature[2]=0;	

	WorkInfo.speed[0]=kotel.fan_out[0];	
	WorkInfo.speed[1]=kotel.fan_out[1];	
	WorkInfo.speed[2]=kotel.smoke_out;		
}
//-----------------------------------------------------------------------------------
