	...
	if(MenuRegs.Manual_Mode)
	{
		...
		case 0:
			...
			else
			{
				if(pos>=(io.num_do))
				{
					if(io.aoutput[pos-io.num_do]<990)io.aoutput[pos-io.num_do]+=10;
					else io.aoutput[pos-io.num_do]=999;
					old_step=step;
					new_step=11;			
					Button_no_update_time=200;	
				}
	...
	if(MenuRegs.Manual_Mode)
	{
		...
		case 0:
			...
			else
			{
				if(pos>=(io.num_do))
				{
					if(io.aoutput[pos-io.num_do]>=10)io.aoutput[pos-io.num_do]-=10;
					else io.aoutput[pos-io.num_do]=0;
					old_step=step;			
					new_step=11;	
					Button_no_update_time=200;	
				}
