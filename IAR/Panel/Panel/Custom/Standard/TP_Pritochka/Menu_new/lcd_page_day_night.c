...
{
	FT_Time=0;
	if(pos_col_new<7)pos_col_new++;		
	if(pos_col_new>4)
		if(pos_row_new>2)pos_row_new=2;	
...
//	u8 col=pos_col>>1;
	...
	if(pos_col<5){
		...
		{
		case 0:
			m.sch_enable[pos_row]=1;
			break;
		case 1:
			m.sch[pos_row][0].Hour++;
			if(m.sch[pos_row][0].Hour>23)m.sch[pos_row][0].Hour=0;			
			break;
		case 2:
			m.sch[pos_row][0].Minute++;
			if(m.sch[pos_row][0].Minute>59)m.sch[pos_row][0].Minute=0;
			break;
		case 3:
			m.sch[pos_row][1].Hour++;
			if(m.sch[pos_row][1].Hour>23)m.sch[pos_row][1].Hour=0;			
			break;
		case 4:
			m.sch[pos_row][1].Minute++;
			if(m.sch[pos_row][1].Minute>59)m.sch[pos_row][1].Minute=0;
			break;
		}
	}
	else{
//		if(pos_col_new==4)
		...
		case 0:
			...
			{
				m.sch_temperature[0]++;
				if(m.sch_temperature[0]>400)m.sch_temperature[0]=400;				
			}
			if(pos_col==7)
			{
				m.sch_temperature[1]++;
				if(m.sch_temperature[1]>400)m.sch_temperature[1]=400;				
			}
			break;
		case 1:
			if(pos_col==5)
			...
			{
				m.sch_fan_speed[0]++;
				if(m.sch_fan_speed[0]>999)m.sch_fan_speed[0]=999;				
			}
			if(pos_col==7)
			{
				m.sch_fan_speed[1]++;
				if(m.sch_fan_speed[1]>999)m.sch_fan_speed[1]=999;				
			}
			break;
		case 2:
			if(pos_col==5)
			...
			{
				m.sch_mode[0]++;
				if(m.sch_mode[0]>1)m.sch_mode[0]=1;				
			}
			if(pos_col==7)
			{
				m.sch_mode[1]++;
				if(m.sch_mode[1]>1)m.sch_mode[1]=1;				
			}
			break;
		}
...
void scr11_minus()
{
	...
	if(pos_col<5){	
		...
		{
		case 0:
			m.sch_enable[pos_row]=0;
			break;
		case 1:			
			if(m.sch[pos_row][0].Hour>0) m.sch[pos_row][0].Hour--;
			else	m.sch[pos_row][0].Hour=23;		
			break;
		case 2:
			if(m.sch[pos_row][0].Minute>0) m.sch[pos_row][0].Minute--;
			else	m.sch[pos_row][0].Minute=59;
			break;
		case 3:
			if(m.sch[pos_row][1].Hour>0) m.sch[pos_row][1].Hour--;
			else	m.sch[pos_row][1].Hour=23;		
			break;
		case 4:
			if(m.sch[pos_row][1].Minute>0) m.sch[pos_row][1].Minute--;
			else	m.sch[pos_row][1].Minute=59;
			break;
		}
		...
		case 0:
			...
			{
				m.sch_temperature[0]--;
				if(m.sch_temperature[0]<100)m.sch_temperature[0]=100;				
			}
			if(pos_col==7)
			{
				m.sch_temperature[1]--;
				if(m.sch_temperature[1]<100)m.sch_temperature[1]=100;				
			}
			break;
		case 1:
			if(pos_col==5)
			...
			{
				m.sch_fan_speed[0]--;
				if(m.sch_fan_speed[0]<0)m.sch_fan_speed[0]=0;				
			}
			if(pos_col==7)
			{
				m.sch_fan_speed[1]--;
				if(m.sch_fan_speed[1]<0)m.sch_fan_speed[1]=0;				
			}
			break;
		case 2:
			if(pos_col==5)
			...
			if(pos_col==6)
			{
				if(m.sch_mode[0]!=0)m.sch_mode[0]=0;				
			}
			if(pos_col==7)
			{
				if(m.sch_mode[1]!=0)m.sch_mode[1]=0;				
			}
			break;					
		}
	}	
...
void print_cursor_y(u8 *str)
{
		...
		case 4:
			...
			break;
		case 5:
			lcd_sprintf(str,17,STR_BASE+4);	
			break;
		case 6:
			lcd_sprintf(str,20,STR_BASE+4);	
			break;
		case 7:
			lcd_sprintf(str,26,STR_BASE+4);	
			break;
		}
}
...
//	s16 temperature;
	...
	lcd_sprintf("Day   Night",19,STR_BASE-2);				
	for(i=0;i<7;i++)
	{
		// First line
		...
		if(m.sch_enable[i])lcd_sprintf("*",5,STR_BASE+i);
		else lcd_sprintf(" ",5,STR_BASE+i);
	...
	// Temperature
	if(m.sch_param_ena[0])lcd_sprintf("*",18,STR_BASE+0);
	else lcd_sprintf(" ",18,STR_BASE+0);
	ShowTemperatureShort(str,&m.sch_temperature[0]);
	lcd_sprintf(str,19,STR_BASE+0);
	
	ShowTemperatureShort(str,&m.sch_temperature[1]);
	lcd_sprintf(str,25,STR_BASE+0);
	// Fan speed
	if(m.sch_param_ena[1])lcd_sprintf("*",18,STR_BASE+1);
	else lcd_sprintf(" ",18,STR_BASE+1);
	ShowPowerShort(str,&m.sch_fan_speed[0]);
	lcd_sprintf(str,19,STR_BASE+1);
	
	ShowPowerShort(str,&m.sch_fan_speed[1]);
	lcd_sprintf(str,25,STR_BASE+1);
	// Modes ON / OFF
	if(m.sch_param_ena[2])lcd_sprintf("*",18,STR_BASE+2);
	else lcd_sprintf(" ",18,STR_BASE+2);
	ByteToStr(str,&m.sch_mode[0],0);
	lcd_sprintf(str,19,STR_BASE+2);
	
	ByteToStr(str,&m.sch_mode[1],0);
	lcd_sprintf(str,25,STR_BASE+2);
