		...
		if((M.error==0)&&(M.rxnew==1))
		{
			...
			switch(M_rx.field.com.bit.com)
			{
				case com_get_stat:		
					...
					if(new_detected==0)new_detected=1;
...
//					device_info[device_found].t=Status_Local->Temperature;
					device_info[device_found].t=Status_Local->T_Room;					
...
/*				case 5:
					...
					if(Current_ID>=MAX_DEVICES_NUM)
					{
						...
						Current_ID=0;	
						lcd_sprintf(">",0,3+device_ptr);	
						if(device_found==0)lcd_sprintf("Nothing found!",7,8);	
					}
					M_SCH_Time=10;
//					step=1;					
				break;				
