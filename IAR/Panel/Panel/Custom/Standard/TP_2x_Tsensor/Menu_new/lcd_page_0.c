...
//--------------------------------------------
__farflash TLCDPict _Fancoil_Off=
{	
	23,88+18,8,
...
*/	
	...
	if(Common_ID_Mode==0){
		// Real temperature
//		temperature=io.ainput[3]+m.adjust_temperature[0];	// Real output temperature
		temperature=T_Sensor+m.adjust_temperature[0];	// Real output temperature
	...
	{		
		...
		{			
			...
			{				
				...
				break;
				case 10:
					// Temperature from Panel sensor
					io.ainput[3]=T_Sensor;
					M_Set_Controller_Register(Current_ID,&io.ainput[3],2);
					step++;					
					M_SCH_Time=10;	
				break;
