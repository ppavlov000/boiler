...
void main()
{
	...
	init();
	sprintf(outtxt,"\n\r"
							"\n\r Panel Boot Loader Ver.04 12.05.2009"
	...
	// Picture part	
	sprintf(outtxt,"\n\r\n\r Checking pictures image...");	
	FGet((u8*)&wtmp,HPLM_FLASH_START+CRC_H_LOC,2);	
	CRC=wtmp;
	wtmp=CheckCRC(4);
	if(wtmp==CRC)
	{
		FGet((u8*)&wtmp,HPLM_FLASH_START+CRC_D_LOC,2);	
		CRC=wtmp;
		wtmp=CheckCRC(2);
		if(wtmp!=CRC)
		{
