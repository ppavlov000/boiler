...
//		tmp=0x0f;	// Disable whatchdog
		...
		tmp=0x02;	// 3.9V level reset
		fram_comp_write(fram_reg_Companion_Control,&tmp,1);
		tmp=0x00;	// Oscilator enabled
...
/*
		...
		// Main program body
		case 2:
			Addr=CODE_ADDR+THEAD_WIDTH;
			fram_mem_read((u16)(CODE_ADDR+LENGTH_LOC),(u8*)&len,4);
//			len=TPARAM_WIDTH-2;
			InitCRC(&crc);
			for(p=0;p<len;p++)	
			{
